"""
BFGS-based optimization for tree packing solutions.

This code is released under CC BY-SA 4.0, meaning you can freely use and adapt it (including
commercially), but must give credit to the original author (Jeroen Cottaar) and keep it under
this license.

Provides BFGS optimizer for local refinement of packing solutions, integrated with PyTorch
L-BFGS implementation for efficient GPU-accelerated gradient descent.
"""

import sys
import os
sys.path.insert(0, os.path.join(os.getcwd(), '../core'))
import kaggle_support as kgs
import cupy as cp
from dataclasses import dataclass
import pack_cost
import copy
from typeguard import typechecked
from torch.utils.dlpack import to_dlpack
import torch


@dataclass
class OptimizerBFGS(kgs.BaseClass):
    """BFGS optimizer for local refinement of packing solutions.
    
    Uses PyTorch's L-BFGS implementation with CuPy arrays via DLPack for efficient
    GPU computation. Optimizes tree positions and boundary parameters jointly.
    """

    cost = None  # Cost function to minimize (CostCompound)
    n_iterations = 100  # Maximum number of BFGS iterations
    history_size = 3  # Number of past gradients to store for Hessian approximation
    max_step = 0.01  # Maximum step size per iteration
    tolerance_rel_change = 0.  # Relative change tolerance for convergence
    stop_on_cost_increase = False  # Stop optimization if cost increases
    use_line_search = False  # Enable strong Wolfe line search

    def __post_init__(self):
        """Initialize default cost function."""
        super().__post_init__()
        # Default: minimize area + boundary distance + overlaps
        self.cost = pack_cost.CostCompound(
            costs=[
                pack_cost.AreaCost(scaling=1e-2),
                pack_cost.BoundaryDistanceCost(scaling=1.),
                pack_cost.CollisionCostOverlappingArea(scaling=1.)
            ]
        )

    @typechecked
    def run_simulation(self, sol: kgs.SolutionCollection):
        """Run BFGS optimization on solution collection.
        
        Parameters
        ----------
        sol : kgs.SolutionCollection
            Solution collection to optimize (not modified in place).
            If sol has n_frozen_prefix > 0, the first n_frozen_prefix trees
            are included in cost computation but their gradients are zeroed
            (frozen obstacles for edge tree relaxation).
            
        Returns
        -------
        kgs.SolutionCollection
            Optimized solution collection (deep copy of input).
        """
        # Validate and prepare solution
        sol.check_constraints()
        sol = copy.deepcopy(sol)
        sol.prep_for_phenotype()

        # Detect frozen prefix (lattice trees as obstacles)
        n_frozen = 0
        frozen_mask = None  # (N_solutions, N_trees) bool mask
        if hasattr(sol, 'get_n_frozen'):
            n_frozen = sol.get_n_frozen()
        if n_frozen > 0:
            frozen_xyt = sol.xyt[:, :n_frozen, :].copy()
            # Build per-solution frozen mask for gradient zeroing
            if hasattr(sol, 'n_inner_array'):
                n_inner_arr = sol.n_inner_array
                N_trees = sol.xyt.shape[1]
                mask_np = np.zeros((sol.N_solutions, N_trees), dtype=bool)
                for i in range(sol.N_solutions):
                    mask_np[i, :n_inner_arr[i]] = True
                # Expand to (N_solutions, N_trees, 1) for broadcasting with (x,y,theta)
                frozen_mask = cp.array(mask_np[:, :, np.newaxis], dtype=bool)

        sol_tmp = copy.deepcopy(sol)
        counter = 0

        # Flatten state into single vector: [xyt_flat, h_flat] per solution
        x0 = cp.concatenate(
            (sol_tmp.xyt.reshape(sol_tmp.N_solutions, -1),
             sol_tmp.h.reshape(sol_tmp.N_solutions, -1)),
            axis=1
        )
        tmp_x = cp.zeros_like(x0, dtype=kgs.dtype_cp)
        tmp_res = cp.zeros_like(x0, dtype=kgs.dtype_cp)
        x0 = torch.from_dlpack(x0.__dlpack__())

        # Pre-allocate temporary buffers
        tmp_xyt = copy.deepcopy(sol.xyt)
        tmp_h = copy.deepcopy(sol.h)
        tmp_cost = cp.zeros(sol.N_solutions, dtype=kgs.dtype_cp)
        tmp_grad = cp.zeros_like(sol.xyt, dtype=kgs.dtype_cp)
        tmp_grad_h = cp.zeros_like(sol.h, dtype=kgs.dtype_cp)

        # Cost and gradient function for L-BFGS
        def f_torch(x, is_main_loop):
            """Compute cost and gradient for L-BFGS optimizer."""
            nonlocal tmp_xyt, tmp_h, tmp_cost, tmp_grad, tmp_grad_h
            nonlocal tmp_res, tmp_x, sol_tmp, counter

            # Convert PyTorch tensor to CuPy array
            tmp_x[:x.shape[0]] = cp.from_dlpack(to_dlpack(x))
            counter += 1
            N_split = sol_tmp.xyt.shape[1] * 3  # Split point between xyt and h
            N = x.shape[0]

            # Unflatten state vector (must go via tmp for contiguity)
            tmp_xyt[:N, :] = tmp_x[:N, :N_split].reshape(N, -1, 3)
            tmp_h[:N, :] = tmp_x[:N, N_split:].reshape(N, -1)

            # Reset frozen trees to original positions (prevent drift)
            if n_frozen > 0:
                tmp_xyt[:N, :n_frozen, :] = frozen_xyt[:N]

            sol_tmp.xyt = tmp_xyt[:N, :]
            sol_tmp.h = tmp_h[:N, :]

            # Compute cost and gradients
            self.cost.compute_cost(
                sol_tmp, tmp_cost[:N], tmp_grad[:N, :], tmp_grad_h[:N, :]
            )

            # Zero gradients for frozen trees (per-solution mask)
            if frozen_mask is not None:
                # frozen_mask is (N_solutions, N_trees, 1), broadcasts over xyz
                tmp_grad[:N] = cp.where(frozen_mask[:N], 0.0, tmp_grad[:N])
            elif n_frozen > 0:
                tmp_grad[:N, :n_frozen, :] = 0.0

            # Flatten gradients to match state vector
            res = cp.zeros_like(tmp_x[:N, :], dtype=kgs.dtype_cp)
            res[:, :N_split] = tmp_grad[:N, :].reshape(sol_tmp.N_solutions, -1)
            res[:, N_split:] = tmp_grad_h[:N, :].reshape(sol_tmp.N_solutions, -1)

            return (
                torch.from_dlpack(tmp_cost[:N].__dlpack__()),
                torch.from_dlpack(res.__dlpack__())
            )

        # Run L-BFGS optimization
        import lbfgs_torch_parallel
        results = lbfgs_torch_parallel.lbfgs(
            f_torch, x0,
            tolerance_grad=0,
            tolerance_change=0,
            tolerance_rel_change=self.tolerance_rel_change,
            max_iter=self.n_iterations,
            history_size=self.history_size,
            max_step=self.max_step,
            line_search_fn='strong_wolfe' if self.use_line_search else None,
            stop_on_cost_increase=self.stop_on_cost_increase
        )

        # Unpack optimized state back to solution
        x_result = cp.from_dlpack(to_dlpack(results))
        sol.xyt = cp.ascontiguousarray(
            x_result[:, :sol.xyt.shape[1] * 3].reshape(sol.N_solutions, -1, 3)
        )
        sol.h = cp.ascontiguousarray(
            x_result[:, sol.xyt.shape[1] * 3:].reshape(sol.N_solutions, -1)
        )

        sol.unprep_for_phenotype()
        return sol


def run_simulation_list(simulator, solution_list):
    """Run simulations on a list of solutions efficiently by batching.
    
    Groups solutions by number of trees, merges each group into a single batch,
    runs simulation once per batch, then scatters results back to individual
    solutions. This exploits GPU parallelism more efficiently than running
    solutions one at a time.
    
    Parameters
    ----------
    simulator : object
        Any object with a run_simulation(sol: kgs.SolutionCollection) method.
    solution_list : list of kgs.SolutionCollection
        Solutions to optimize. All solutions in each group must be the same
        subclass with matching properties (except xyt and h). Modified in-place.
        
    Notes
    -----
    - Solutions are grouped by N_trees
    - Each group is merged, optimized together, then scattered back
    - In-place modification: solution_list[i].xyt and .h are updated
    """
    if len(solution_list) == 0:
        return

    # Group solutions by number of trees
    from collections import defaultdict
    groups = defaultdict(list)

    for sol in solution_list:
        N_trees = sol.xyt.shape[1]
        groups[N_trees].append(sol)

    # Process each group separately
    for N_trees, group_sols in groups.items():
        # Single solution: optimize directly without merge/scatter
        if len(group_sols) == 1:
            result = simulator.run_simulation(group_sols[0])
            group_sols[0].xyt[:] = result.xyt
            group_sols[0].h[:] = result.h
            continue

        # Verify compatibility in debug mode
        if kgs.debugging_mode >= 2:
            first_type = type(group_sols[0])
            for sol in group_sols[1:]:
                if type(sol) is not first_type:
                    raise ValueError(
                        f"Solution type mismatch: {first_type} vs {type(sol)}"
                    )

            # Check properties match (excluding xyt and h)
            first_sol = group_sols[0]
            for i, sol in enumerate(group_sols[1:], 1):
                first_sol_copy = copy.deepcopy(first_sol)
                sol_copy = copy.deepcopy(sol)

                first_sol_copy.xyt = None
                first_sol_copy.h = None
                sol_copy.xyt = None
                sol_copy.h = None

                if first_sol_copy != sol_copy:
                    raise ValueError(
                        f"Solution {i} properties mismatch with first solution"
                    )

        # Merge solutions along batch dimension
        merged_xyt = cp.concatenate([sol.xyt for sol in group_sols], axis=0)
        merged_h = cp.concatenate([sol.h for sol in group_sols], axis=0)

        merged_sol = copy.deepcopy(group_sols[0])
        merged_sol.xyt = merged_xyt
        merged_sol.h = merged_h

        # Merge any per-solution metadata (e.g. lattice_params)
        if hasattr(merged_sol, 'lattice_params') and merged_sol.lattice_params is not None:
            merged_sol.lattice_params = cp.concatenate(
                [sol.lattice_params for sol in group_sols], axis=0
            )

        # Run simulation on merged batch
        result = simulator.run_simulation(merged_sol)

        # Scatter results back to original solutions
        start_idx = 0
        for sol in group_sols:
            N_sol = sol.xyt.shape[0]
            sol.xyt[:] = result.xyt[start_idx:start_idx + N_sol]
            sol.h[:] = result.h[start_idx:start_idx + N_sol]
            start_idx += N_sol
